...
"header": {
  ...
  "publicKeyHash": "/4UKqrtx7AmlRvLatYt9LDt64IYo+G9eaqqS6LFOAdI="
        ...
        val json = applePayInfo.toJson()
        val paymentMethod = json["payment_method"] as JsonObject
        val applePay = paymentMethod["apple_pay"] as JsonObject
        val paymentData = applePay["payment_data"] as JsonObject
        val header = paymentData["header"] as JsonObject
        assertEquals(
            expected = "Jane",
            actual = paymentMethod["first_name"]!!.jsonPrimitive.content,
        )
        assertEquals(
            expected = "Doe",
            actual = paymentMethod["last_name"]!!.jsonPrimitive.content,
        )
        assertEquals(
            expected = "EC_v1",
            actual = paymentData["version"]!!.jsonPrimitive.content,
        )
        assertEquals(
            expected = "/4UKqrtx7AmlRvLatYt9LDt64IYo+G9eaqqS6LFOAdI=",
            actual = header["publicKeyHash"]!!.jsonPrimitive.content,
        )
    }
}
